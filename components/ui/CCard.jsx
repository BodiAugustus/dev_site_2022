...
const CryptoCard = () => {
  ...
  return (
    <div
      className="flex flex-col flex-1 items-center justify-start w-full mt-8
        md:mt-0 
        "
    >
      <div
        className="p-3 justify-end items-start flex-col rounded-xl h-40 my-5 bg-slate-400 eth-card white-glassmorphism
        xs:w-80 
        sm:h-48 sm:w-[90%] 
        md:h-60 md:w-2/3 
      ...
      >
        <div className="flex justify-between flex-col w-full h-full">
          <div className="flex justify-between items-start">
            <div
              className="w-12 h-12 rounded-full border-2 border-white flex justify-center items-center text-white tracking-wide
                    md:w-16 md:h-16 
                    lg:w-14 lg:h-14 "
            >
          ...
          <div>
            <p className="text-white font-light text-sm ms:text-xl md:text-2xl lg:text-base xl:text-xl">
              ...
              {account.data
                ? account.isAdmin
                  ? `Bodi!`
                  : ` ${account.data.slice(0, 6)}...${account.data.slice(36)}!`
                : 'Not Connected - Connect to MetaMask'}
              {/* Hi {account.data} */}
            </p>
            <p className="text-white font-medium text-md ms:text-lg md:text-xl mt-1 lg:text-sm xl:text-lg">
              Currently on{' '}
